'use client';

import React from 'react';
import {
  X,
  Globe,
  Link as LinkIcon,
  ExternalLink,
  Phone,
  Mail,
  MapPin,
  Store,
  ShoppingBag,
  Headphones,
  Star,
  Heart,
  Home,
  Info
} from 'lucide-react';

// Custom SVGs for brand icons not present in this lucide-react version
const InstagramIcon = ({ size, ...props }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}>
    <rect width="20" height="20" x="2" y="2" rx="5" ry="5"/>
    <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/>
    <line x1="17.5" x2="17.51" y1="6.5" y2="6.5"/>
  </svg>
);

const FacebookIcon = ({ size, ...props }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}>
    <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/>
  </svg>
);

const YoutubeIcon = ({ size, ...props }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}>
    <path d="M2.5 17a24.12 24.12 0 0 1 0-10 2 2 0 0 1 1.4-1.4 49.56 49.56 0 0 1 16.2 0A2 2 0 0 1 21.5 7a24.12 24.12 0 0 1 0 10 2 2 0 0 1-1.4 1.4 49.55 49.55 0 0 1-16.2 0A2 2 0 0 1 2.5 17z"/>
    <polygon points="10 15 15 12 10 9 10 15"/>
  </svg>
);

const LinkedinIcon = ({ size, ...props }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}>
    <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/>
    <rect width="4" height="12" x="2" y="9"/>
    <circle cx="4" cy="4" r="2"/>
  </svg>
);

const customIconMap = {
  instagram: InstagramIcon,
  facebook: FacebookIcon,
  youtube: YoutubeIcon,
  linkedin: LinkedinIcon,
};

const lucideIconMap = {
  twitter: X,
  'x/twitter': X,
  'google-maps': MapPin,
  globe: Globe,
  link: LinkIcon,
  'external-link': ExternalLink,
  phone: Phone,
  mail: Mail,
  'map-pin': MapPin,
  store: Store,
  'shopping-bag': ShoppingBag,
  support: Headphones,
  star: Star,
  heart: Heart,
  home: Home,
  info: Info
};

export default function SocialIcon({ iconKey, size = 18, className = '', ...props }) {
  const normalizedKey = iconKey?.toLowerCase();
  
  if (customIconMap[normalizedKey]) {
    const CustomIcon = customIconMap[normalizedKey];
    return <CustomIcon size={size} className={className} {...props} />;
  }
  
  const LucideIconComponent = lucideIconMap[normalizedKey] || LinkIcon;
  return <LucideIconComponent size={size} className={className} {...props} />;
}
